...
#schardict = list(chardict.keys())
schardict = []
for j in chardict.keys():
    schardict.append(j)
schardict.sort()
scd = {i: chardict[i] for i in schardict}
print(scd) # scd is the sorted dictionary pairs
...
for u in modict.keys():
    print(f"The letter that occurred the most is {u}")
print("*"*75)
# Output which letter occurred the least 
for y in lodict.keys():
    print(f"The letter that occurred the least is {y}")
print("*"*75)

# Output what the percentage of the string each character is, again in alphabetical
 #/25 will give the percent of each i e 103/25 is 4.12%, which makes sense as 103/2500 is 4.12% or .0412.
 #scd is the list from the start that has each pair and number so...
for key1, value1 in scd.items():
    pervalue = value1/round(len(random_string)/100, 2)
    print(f"{key1} occurs {value1} times, which is {round(pervalue, 2)}% of the total random string.")
print("You can also change the size of the list or add characters and it will still work :)")